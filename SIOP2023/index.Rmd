...
```{r simulatedgraphs, results="hide", fig.cap="Shape of prescribed distributions of *p*-values across Study 1 conditions.", fig.height=6, fig.width=16}
...
```{r histrogram, results="hide", fig.height=6, fig.width=15}
#Area between curves
#Preparing data
library(geiger)
citcs<-data.frame(alphas$item.stats$r.drop)
...
```{r histrogram2, results="hide", fig.cap="Histogram of all areas between ICCs plotted using IRT parameters vs ICCs plotted using CTT parameters.", fig.height=6, fig.width=15}
...
auc<-unlist(auc)
h2<-hist(auc, color="cyan", xlim=c(0,3))
